const loadUserDetails = () => {
  ...
  fetch("https://bechedao-backend.onrender.com/user/list/")
    .then((res) => res.json())
    .then((data) => {
    ...
    .then((data) => {
      ...
      data.results?.forEach((recipie) => {
        ...
        if(recipie.user == user) {
          ...
          console.log("last", recipie.name);
          div.innerHTML = `
          
          <div class="card text-center">
          <img src=${recipie.image} class="card-img-top w-100" alt="...">
        <div class="card-header">
          <h5 class="card-title">Name: ${recipie.name}</h5>
        
        </div>
        <div class="card-body">
          <p class="card-title">Category: ${categoryNames}</p>
          <p class="card-text">Ingredients: ${recipie.description.slice(0, 30)}...</p>
          <p class="card-text"> Price: ৳{recipie.price} </p>
          <p class="card-text"> Condition: ${recipie.condition} </p>
